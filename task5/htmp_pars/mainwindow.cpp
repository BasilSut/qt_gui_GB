...
{
        QString filename = QFileDialog::getOpenFileName(this, tr("Open file"), QDir::current().path(), tr("Webpage, HMTL files (*.html *.htm);;All files(*.*)"));
		...
		if(HTML_file.open(QIODevice::ReadOnly)){
			...
			try {
				...
				ui->lineEdit_EUR->setText(results[1]);
                    ...
                    ui->lineEdit_USD->setText(tr("Cannot parse this file"));
                    ui->lineEdit_EUR->setText(tr("Cannot parse this file"));
			}
			...
			try {
                ...
                ui->plainTextEdit->appendPlainText(QString(tr("Cannot parse this file")));
			}
